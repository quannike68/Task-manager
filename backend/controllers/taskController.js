...
const deleteTask = async (req, res) => {
  try {
    ...
    if (deleteTask) {
      return res
        .status(200)
        .json({ message: `Delete task: ${task.title} success` });
    }
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
  ...
  try {
    ...
    if (updateStatus) {
      return res.status(200).json({
        message: `update status task: ${task.title} success`,
        updateStatus,
      });
    }
  } catch (error) {
...
const updateTaskCheckList = async (req, res) => {
  try {
    ...
    // ✅ Xử lý nếu assignedTo là mảng hoặc object
    ...
    if (!assignedUserIds.includes(currentUserId) && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Not authorized to update this task" });
    }

    // ✅ Cập nhật checklist và tiến độ
...
const dashboardData = async (req, res) => {
  try {
    ...
    const priorityTasksCount = priorityTasks.reduce((acc, priority) => {
      acc[priority] =
        priorityTasksRaw.find((item) => item._id === priority)?.count || 0;
      return acc;
    }, {});
    ...
    const recentTasks = await Task.find({})
      ...
      .limit(10)
      .select(
        "title status priority dueDate assignedTo createBy createdAt updatedAt "
      );
